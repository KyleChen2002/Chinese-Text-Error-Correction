...
def args_parse(config_file=''):
    ...
    if config_file != '':
        ...
        with open("D:/pycorrector/pycorrector/macbert/train_macbert4csc.yml", 'r') as cf:
            config_str = "\n" + cf.read()
...
def main():
    ...
    # 加载之前保存的模型，继续训练
    print(f'debug ====  加载.  ${cfg.MODEL.WEIGHTS}')
    if cfg.MODEL.WEIGHTS and os.path.exists(cfg.MODEL.WEIGHTS):
        print(f'debug ==== 加载之前保存的模型，继续训练  {cfg.MODEL.WEIGHTS}')
